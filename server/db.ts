...
// ============================================================================
// Trading Coach — AI setup analyses history.
// ============================================================================
import { desc } from "drizzle-orm";
import { coachAnalyses, type InsertCoachAnalysis } from "../drizzle/schema";

export async function createCoachAnalysis(row: InsertCoachAnalysis) {
  const db = await getDb();
  if (!db) return undefined;
  const [res] = await db.insert(coachAnalyses).values(row).$returningId();
  if (!res) return undefined;
  const [created] = await db
    .select()
    .from(coachAnalyses)
    .where(eq(coachAnalyses.id, res.id))
    .limit(1);
  return created;
}

export async function listCoachAnalyses(userId: number, limit = 30) {
  const db = await getDb();
  if (!db) return [];
  return db
    .select()
    ...
    .orderBy(desc(coachAnalyses.createdAt))
    .limit(limit);
}
